...
const ChatCustomAction = ({ content }: ChatCustomActionProps) => {
  ...
  const handleSubmit = () => {
    ...
    handleDialogClose()
    toast.push(
      <Notification type="success">Thanks for your feedback!</Notification>,
    ...
    <>
      <div className="flex mt-0.5">
        ...
        <Tooltip title="Good response" placement="bottom">
          <button
            className={classNames(btnClass, 'text-lg')}
            onClick={() => setResponseDialog({ type: 'praise', open: true })}
          >
            {responseSend === 'praise' ? <TbThumbUpFilled /> : <TbThumbUp />}
          </button>
        </Tooltip>
        <Tooltip title="Bad response" placement="bottom">
          <button
            className={classNames(btnClass, 'text-lg')}
            onClick={() => setResponseDialog({ type: 'blame', open: true })}
          >
            {responseSend === 'blame' ? <TbThumbDownFilled /> : <TbThumbDown />}
          </button>
        </Tooltip>
      </div>
      <Dialog
              ...
              <FormItem label="What type of issue do you wish to report? (optional)">
                <Select
                  ...
                  value={responseOption.filter(
                    (response) => response.value === selected,
                  )}
