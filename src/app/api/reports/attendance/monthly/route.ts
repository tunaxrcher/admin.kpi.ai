...
export const GET = withErrorHandling(async (request: NextRequest) => {
  ...
  // สร้าง Set ของวันหยุดเพื่อการค้นหาที่เร็วขึ้น
  ...
  const reports = characters.map((character) => {
    // กรองข้อมูลการเข้าออกงานของตัวละครนี้
    const characterAttendance = attendanceRecords.filter(
      (record) => record.characterId === character.id,
    )

    // สร้าง Set ของวันทำงาน
    const workDaysSet = new Set((character.workDays as number[]) || [])

    // สร้างรายการวันในเดือน
    const daysInMonth = eachDayOfInterval({ start: monthStart, end: monthEnd })

    // นับจำนวนวันทำงานและการเข้าออกงาน
    let workDaysCount = 0
    let checkinDays = 0
    let checkoutDays = 0
    let lateDays = 0
    ...
    // สร้างข้อมูลวันทำงาน
    const workDaysData = daysInMonth.map((day) => {
      ...
      return {
        ...
        holidayName: isHoliday
          ? holidays.find((h) => format(h.date, 'yyyy-MM-dd') === dateKey)?.name
          : null,
      }
    })
